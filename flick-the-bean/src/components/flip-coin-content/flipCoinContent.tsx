    ...
    if (wallet == "xverse") {
      ...
      if (publicKey != "" && signature != "") {
        ...
        if (gameResponse != undefined && newBalance != "0.00") {
          setTimeout(() => {
            setStartAnimation(
              `coin_${gameResponse ? winRandom : lostRandom}.gif`
            );
			if(gameResponse){
				playWinAudio();
			  } else {
    ...
    <>
      ...
      <section className="btns-wrapper">
        ...
        <div className="result h-100">
          {loading && (
            <img
              className="coin-start-animation"
        ...
        <div className="btns-inner-wrapper">
          ...
          <div
            className={`btns-verification ${
              !loading && !first && "btns-verification-active"
            }`}
          ...
          >
            <div className="btns-verification-icon">
              <img
                src={`/static/img/${
                  !loading && !first ? "tick_active.png" : "tick_inactive.png"
                }`}
              />
            </div>
            <span>VERIFY</span>
          ...
          <div className="btns-display">
            <div className="btns-display-points">
              <div className="btns-display-points-title">XP points</div>
            ...
            <div className="btns-display-recent">
              ...
              {data?.length ? (
                <div className="btns-display-recent-value">
                  {data.map((item) => (
                    <div className="btns-display-recent-value-row">
                      <div className="amount">
                        <span className="balance">
                          {item?.public_key.slice(0, 5)}...
                          {item?.public_key.slice(-5)}
                        </span>{" "}
                        just flipped{" "}
                        <span className="bold">
                          {Math.round(
                            (parseFloat(item.bet_amount) + Number.EPSILON) * 100
                          ) / 100}{" "}
                          ACD3
                        </span>{" "}
                        and <span className={item.outcome}>{item.outcome}</span>
                      </div>
                    </div>
                  ))}
                </div>
